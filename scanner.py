...
#
#       s.readtoken()       # read the next whitespace delimited token
#       s.readchar()        # read the next non-whitespace character
#       s.readrawchar()     # read the next character (whitespace or not)
#       s.readbool()        # read a Boolean value
#       s.readint()         # read an integer
#       s.readfloat()       # read a real number
#                           # does not read scientific notation, like 2e10
#       s.readstring()      # read a double-quote delimited string
#                           # the double quotes are left on the string
#       s.readline()        # read the rest of the current or the next line
#                           # if at the end of the current line
#       s.setWhiteSpace(t)  # set what the scanner considers whitepace to t
#       s.pushBack(ch)      # put ch back on the input to be read again
#
...
class Scanner:
    ...
    def _getToken(self):
        ...
        str = ''
        while ch != '' and not (ch.isspace() or ch == ','):
            str += ch
            ch = self._getNextCharacter()
    ...
    def _getString(self):
        ...
        if (delimiter == ''): return ''
        if (delimiter != '\'' and delimiter != '"'): return ''
    ...
    def _skipWhiteSpace(self):
        ch = self._getNextCharacter()
        while (ch != '' and self.isWhiteSpace(ch)):
            ch = self._getNextCharacter()
        self._pushBack(ch)
